module.exports = {
    ...
    var guildapi = await apikey.get(guild)
		console.log("Guild API Key is: " + guildapi)
    var guildSID = await statusID.get(guild)
		console.log("GuildSID Is: " + guildSID)
    var guildSCH = await statusCH.get(guild)
		console.log("GuildSCH is: " + guildSCH)
  if (!(guildapi)) {
  ...
  if ((guildapi)) {
  if ((guildSID)) {
		status(client,message,(message.channel),guildapi,guildSCH,guildSID);
}
}
	},
};
...
async function status(client,message,channel,guildapi,guildSCH,guildSID){
