module.exports = {
	...
	usage: 'request (3 letter id from https://www.weather.gov/ EX: okx (New York City))',
	guildOnly: true,
	//code to be executed
async	execute(message, args, displayColor, client) {
		...
		var char = /[a-z][a-z][a-z]/g
			...
			loadchannel.send({embed: {
				...
				message.channel.send({embed: {
					...
					image: {url: `https://radar.weather.gov/lite/N0R/${temp}_loop.gif`}

					}});
			}
			else{
				message.channel.send("Invalid region code! Must be 3 letter code from https://www.weather.gov/ !")
			}
		break;
		default:
		...
		message.channel.send({embed: {
			...
			image: {url: `https://radar.weather.gov/lite/N0R/${region}_loop.gif`}

		}});}
		else{
			message.channel.send("No Region has been set for this server! To set a region do /weather set (region code) . Region codes can be found here: https://www.weather.gov/")
		}
		break;
}
	},
